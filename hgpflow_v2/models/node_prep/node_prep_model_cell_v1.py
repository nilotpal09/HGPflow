...
class NodePrepModelCellV1(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pre_node_transformer = DiTEncoder(**config['pre_node_transformer'])
    ...
    def forward(self, batch):
        ...
        prenode_to_prenode_mask = batch['prenode_to_prenode_edge_mask']

        # transformer on pre_node
        pre_node_global = pre_node_feat.sum(dim=1) / \
            pre_node_mask.sum(dim=1, keepdim=True)
        pre_node_feat = self.pre_node_transformer(
            q=pre_node_feat, q_mask=~pre_node_mask, 
            attn_mask=prenode_to_prenode_mask,
            context=pre_node_global)
        ...
        if hasattr(self, 'node_transformer'):
            ...
            node_feat = self.node_transformer(q=node_feat, context=pre_node_global)
