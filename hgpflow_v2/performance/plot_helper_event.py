...
def compute_jet_residual_dict(matched_jets_dict, dr_cut=0.1, leading_N_jets=999, pt_min=10, eta_max=2.5):
    ...
    for name in matched_jets_dict.keys():
        residual_dict[name] = {
            'pt': [], 'pt_rel': [], 'eta': [], 'phi': [], 'dR': [], 'ref_pt': [], 'ref_eta': [], 'nconst': [],
            'e': [], 'e_rel': [], 'ref_e': []
        }
...
def plot_jet_residuals(residual_dict, pt_relative=True, stylesheet=None, separate_figures=False):
    ...
    for v_i, var in enumerate(vars):
        ...
        for name, res in residual_dict.items():
            if var == 'dR':
                ax.hist(res[var], bins=bins, histtype=_HISTTYPES[name], label=_LABELS[name], 
                    color=_COLORS[name], linestyle=_LINE_STYLES[name], alpha=_ALPHAS[name])
            else:
                custom_hist_v2(ax, res[var], label_length=_LABEL_LEN[name], metrics="mean std iqr", 
                    f=residual_dict[name]['f_matched'], bins=bins, histtype=_HISTTYPES[name], 
                    label=_LABELS[name], color=_COLORS[name], linestyle=_LINE_STYLES[name], alpha=_ALPHAS[name])
    ...
    for pt_i, (pt_min, pt_max) in enumerate(zip(pt_mins, pt_maxs)):
        ...
        for name, res_vals in res_dict_pt_i.items():
            ...
            try:
                ...
                fit_worked=False
                a1=np.nan; mu1=np.nan; sigma1=np.nan; a2=np.nan; mu2=np.nan; sigma2=np.nan

            label = f'{_LABELS[name]} {a1:.3f} * N({mu1:.3f}, {sigma1:.3f}) + {a2:.3f} * N({mu2:.3f}, {sigma2:.3f})'
            hist, _, _ = axs_fit[pt_i].hist(res_vals, bins=bins, density=False, label=label, color=_COLORS[name],
                                            alpha=_ALPHAS[name], histtype=_HISTTYPES[name])
...
def plot_jet_marginals(jet_dict, nleading=1, stylesheet=None):
    ...
    for var_i, var in enumerate(['pt', 'pt', 'eta', 'phi']):
        ...
        for name_i, (name, var_dict) in enumerate(jet_dict_mod.items()):
            ax.hist(var_dict[var], bins=_bins[var], histtype=_HISTTYPES[name], label=_LABELS[name], 
                color=_COLORS[name], alpha=_ALPHAS[name])
